...
def exact_omr_result(
    ...
    *,
    ...
    if show:
        ...
        axes[0, 0].imshow(cv2.cvtColor(out, cv2.COLOR_BGR2RGB))
        axes[0, 0].set_title("Result (perspective-normalized to template)")
        axes[0, 0].axis("off")

        coaching_crop_view = cv2.resize(coaching_crop, (300, 400), interpolation=cv2.INTER_AREA)
        axes[0, 1].imshow(cv2.cvtColor(coaching_crop_view, cv2.COLOR_BGR2RGB))
        axes[0, 1].set_title("Coaching square (crop)")
        axes[0, 1].axis("off")
        ...
        axes[2, 0].imshow(cv2.cvtColor(set_result.set_crop_view, cv2.COLOR_BGR2RGB))
        axes[2, 0].set_title(f"SET box (zoom) | set_label={set_result.set_label}")
        axes[2, 0].axis("off")
